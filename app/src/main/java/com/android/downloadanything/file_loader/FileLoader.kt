...
import com.android.downloadanything.file_loader.FilesUtils
import java.io.File
import java.util.*
...
class FileLoader (val context: Context)  {
    ...
    init {
        ...
        executorService = Executors.newFixedThreadPool(5, FilesUtils.ImageThreadFactory())
        handler = Handler()
    ...
    private  fun loadImageIntoImageView(imageView: ImageView, bitmap: Bitmap?, imageUrl: String) {
        ...
        val scaledBitmap = FilesUtils.scaleBitmapForLoad(bitmap, imageView.width, imageView.height)
    ...
    inner class PhotosLoader(private var imageRequest: ImageRequest) : Runnable {
        ...
        override fun run() {
            ...
            val bitmap = FilesUtils.downloadBitmapFromURL(imageRequest.imgUrl)
